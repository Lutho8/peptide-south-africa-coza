...
import { peptides, getCategoryLabel, Peptide } from '@/data/peptides';
import { GradientCard } from '@/components/ui/GradientCard';
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { PeptideDetailModal } from '@/components/modals/PeptideDetailModal';
import { Search, ExternalLink, BookOpen, FlaskConical, Calendar, Users, Database, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';

type FilterOption = 'all' | string;
type ViewMode = 'research' | 'peptides';
  ...
  return (
    <div className="pb-24 space-y-4 fade-in">
      ...
      <p className="text-sm text-muted-foreground">
        {researchReferences.length} peer-reviewed papers • {peptides.length} peptides in database
      </p>
      ...
      <div className="relative">
        ...
        <Input
          placeholder={viewMode === 'research' ? "Search papers, authors, or findings..." : "Search peptides..."}
          value={searchQuery}
              ...
              <div className="flex flex-wrap gap-1.5">
                {ref.peptideIds.map((peptideId) => {
                  ...
                  return (
                    <button
                      key={peptideId}
                      onClick={() => peptide && handleViewPeptide(peptide)}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-primary/10 border border-primary/30 text-primary hover:bg-primary/20 transition-colors"
                    >
          ...
          {filteredPeptides.map((peptide) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/50">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">Dosing (Intermediate)</p>
                  <p className="text-sm font-semibold text-foreground">{peptide.dosing.intermediate}</p>
                </div>
              </div>
